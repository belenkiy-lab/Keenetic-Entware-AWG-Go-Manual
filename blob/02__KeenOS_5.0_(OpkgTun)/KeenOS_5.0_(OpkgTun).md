...
Переходим в настройки маршрутизации по DNS и добавляем список доменных имён:  
![WebUI DNS Routes](!img/05__Go-to_DNS-Routes.png)  

Для примера создадим список из 3х сайтов:  
![WebUI DNS Routes list add](!img/06__Create_routing_list.png)  

Теперь добавляем непосредственно параметры маршрута:  
![WebUI DNS Routes add](!img/07__Create_routing_rules.png)  
Где:
Название                | Значение
----------------------- | ----------------------------------------
Имя списка              | Список доменных имён, IP адресов и диапазонов;
Интерфейс               | Куда будут перенаправляться запросы из соответствующего списка.
Добавлять автоматически | бла-бла-бла
Эксклюзивный маршрут    | бла-бла-бла
